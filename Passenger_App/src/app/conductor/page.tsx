...
export default function ConductorTerminalPage() {
  const serviceLinks = [
    {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col" suppressHydrationWarning>
      <Header title="CONDUCTOR TOOLS" showBackButton={true} backHref="/" />
      
      <main className="flex-grow flex flex-col items-center pt-16 p-4">
        <h1 className="text-2xl font-bold text-slate-900 mb-10 font-headline">Conductor Dashboard</h1>
        
        <div className="w-full max-w-2xl space-y-4">
          {serviceLinks.map((link) => (
            <Link href={link.href} key={link.title} className="group block no-underline">
              <Card className="hover:shadow-md transition-all border-slate-100 rounded-xl bg-white overflow-hidden shadow-sm">
